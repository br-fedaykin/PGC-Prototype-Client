...
public class BlockchainConnection {
    ...
    public BlockchainConnection(String networkURL, Map<String, String> contractAddress) {
        ...
        ethGasPrice = web3j.ethGasPrice();
        ethLatestBlock = web3j.ethGetBlockByNumber(DefaultBlockParameterName.LATEST, false);
    ...
    public Map<String, int[]> publishAttributeRequestUpdate(String authority, String user, BigInteger pendingRequesterIndex,
        ...
        try {
            ...
            if (profiler.isEnabled()) {
                profiler.addGasCost(tx.getGasUsed(), ethGasPrice.send().getGasPrice());
                profiler.saveGasLimit(ethLatestBlock.send().getBlock().getGasLimit());
            }
